...
public class AI : MonoBehaviour
{
    ...
    private bool isAware = false; // estado do inimigo
    ...
    public void Update()
    {
        ...
        #region Visao
        if (isAware && Vector3.Distance(player.transform.position, transform.position) < viewDistance
         && Vector3.Angle(Vector3.forward, transform.InverseTransformPoint(player.transform.position)) < fov / 2 && hit.transform.CompareTag("Player"))
        {
            //Chase player
        ...
        #region Audio
        if (isAware && Vector3.Distance(player.transform.position, transform.position) < soundDistanceParado
         && Vector3.Angle(Vector3.forward, transform.InverseTransformPoint(player.transform.position)) < fos
         || isAware && Vector3.Distance(player.transform.position, transform.position) < soundDistanceAndandoAbaixado
         && Vector3.Angle(Vector3.forward, transform.InverseTransformPoint(player.transform.position)) < fos
         || isAware && Vector3.Distance(player.transform.position, transform.position) < soundDistanceAndando
         && Vector3.Angle(Vector3.forward, transform.InverseTransformPoint(player.transform.position)) < fos
         || isAware && Vector3.Distance(player.transform.position, transform.position) < soundDistancePulando
         && Vector3.Angle(Vector3.forward, transform.InverseTransformPoint(player.transform.position)) < fos
         || isAware && Vector3.Distance(player.transform.position, transform.position) < soundDistanceCorrendo
         && Vector3.Angle(Vector3.forward, transform.InverseTransformPoint(player.transform.position)) < fos
         || isAware && Vector3.Distance(player.transform.position, transform.position) < soundDistanceInteragindoComACaixa
         && Vector3.Angle(Vector3.forward, transform.InverseTransformPoint(player.transform.position)) < fos)
        ...
        if (player.somDoPersonagem <= player.somDoPersonagemParado)
        {
    ...
    {
        if (isAware == false && player.isCrounching == false)
        {
            if (Vector3.Angle(Vector3.forward, transform.InverseTransformPoint(player.transform.position)) < fov / 2)
            {
                if (Vector3.Distance(player.transform.position, transform.position) < viewDistance)
                {
                    if (Physics.Linecast(transform.position, visaoDaCabeca.transform.position, out hit, -1))
                    {
        ...
        else if (isAware == false && player.isCrounching == true)
        {
            if (Vector3.Angle(Vector3.forward, transform.InverseTransformPoint(player.transform.position)) < fov / 2)
            {
                if (Vector3.Distance(player.transform.position, transform.position) < viewDistance)
